...
#              y aislamiento de archivos por usuario en cascada.
# ===========================================================================
...
C_BORDE   = Fore.LIGHTBLACK_EX + Style.BRIGHT  # Gris oscuro/Negro para la interfaz
...
# Archivo maestro de credenciales (El registro de acceso al sistema)
...
def confirmar_accion(mensaje):
    """
    Guardia de confirmation booleana (Sí/No).
...
def registrar_usuario():
    ...
    limpiar()
    print(C_BORDE + "╔" + "═"*44 + "╗")
    print(C_BORDE + "║" + C_TITULO + "          REGISTRO DE NUEVO USUARIO         " + C_BORDE + "║")
    print(C_BORDE + "╚" + "═"*44 + "╝")
    ...
    while True:
        ...
        if confirmar_accion(f"¿Crear credenciales para '{username}'?"):
            # Encriptación segura: Transforma el string a bytes, aplica sal y hashea.
            pwd_bytes = password.encode('utf-8')
            salt = bcrypt.gensalt()
            hashed_pwd = bcrypt.hashpw(pwd_bytes, salt).decode('utf-8')
            
            guardar_usuario(username, hashed_pwd)
            print(f"\n{C_EXITO}✅ Usuario '{username}' registrado. Credenciales encriptadas exitosamente.")
            time.sleep(2)
            break
...
def iniciar_sesion():
    ...
    usuarios_db = cargar_usuarios()
    if not usuarios_db:
        print(f"\n{C_ERROR}⚠️ No hay usuarios registrados en el sistema. Regístrese primero.")
        time.sleep(2)
        return None
...
# ============================================================
#               BASE DE DATOS DE TAREAS (PERSISTENCIA I/O)
# ============================================================
...
def obtener_archivo_usuario(username):
    ...
    leer ni mutar las tareas de otro usuario en el sistema.
    """
    return f"tareas_{username}.txt"
...
def listar_tareas(tareas, mostrar_vacias=True):
    ...
    if not tareas:
        if mostrar_vacias:
            print(f"\n{C_INFO}📭 La lista de tareas está vacía. ¡Agrega una nueva!")
        return False
...
def eliminar_tarea(username, tareas):
    ...
    while True:
        ...
        if confirmar_accion(f"¿Estás ABSOLUTAMENTE seguro de eliminar '{tarea_actual['nombre']}'?"):
            ...
            print(f"\n{C_INFO}Operación cancelada. La tarea está a salvo en el sistema.")
            time.sleep(1.5)
            break

# ============================================================
#               INTERFAZ PANEL PRINCIPAL (SESIÓN INICIADA)
# ============================================================
...
def menu_principal(username):
    ...
    while True:
        ...
        print(C_BORDE + "╔═══════════════════════════════════════════╗")
        print(C_BORDE + "║" + f" {C_TITULO}📝 TAREAS DE: {username.upper()}".ljust(51) + C_BORDE + "║")
        print(C_BORDE + "╠═══════════════════════════════════════════╣")
        print(C_BORDE + "║ " + f"{C_INFO}📊 ESTADO DEL SISTEMA:{C_BORDE}".ljust(48) + "           ║")
        ...
        if op == "1":
            ...
            input(f"\n{C_BORDE}[ENTER para volver al panel principal]{Style.RESET_ALL}")
        elif op == "2":
            ...
            if confirmar_accion("¿Cerrar sesión y asegurar tus tareas?"):
                limpiar()
...
# ============================================================
#               PUNTO DE ENTRADA (EXTERIOR DEL GESTOR)
# ============================================================

def menu_acceso():
    """
    Capa externa de la aplicación.
    Maneja el routing principal antes de permitir acceso a cualquier base de datos de tareas.
    Actúa como el primer muro defensivo del sistema.
    """
    while True:
        limpiar()
        print(C_BORDE + "╔═══════════════════════════════════════════╗")
        print(C_BORDE + "║" + C_TITULO + "         🔒 GESTOR DE AUTENTICACIÓN         " + C_BORDE + "║")
        print(C_BORDE + "╚═══════════════════════════════════════════╝")
        print(f"{C_INFO} 1.{C_TEXTO} Iniciar Sesión")
        print(f"{C_INFO} 2.{C_TEXTO} Registrar Nuevo Usuario")
        print(f"{C_ERROR} 3.{C_TEXTO} Apagar Sistema")
        
        op = input(f"\n{C_INPUT}Seleccione su opción: {Style.RESET_ALL}").strip()
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n\n{C_EXITO}🔒 Interrupción detectada. Guardando cambios y saliendo de forma segura...")
        time.sleep(1)
